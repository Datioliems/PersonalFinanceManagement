<?php
// ============================================================
// VIEW — app/Views/incomes/index.php
// ============================================================
// Biến nhận từ IncomeController::index():
//   $items, $pager, $filterMonth, $sort,
//   $totalIncome, $monthNow, $yearNow
// ============================================================
$pageTitle = $pageTitle ?? 'Thu nhập';
require BASE_PATH . '/app/Views/partials/layout.php';
?>

<!-- Card tóm tắt tổng thu tháng -->
        ...
        <div class="card border-0 bg-success bg-opacity-10 h-100">
            <div class="card-body">
                <div class="small text-muted mb-1">
                    ...
                    Tổng thu tháng <?= $monthNow ?>/<?= $yearNow ?>
                </div>
...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h2 class="h4 fw-semibold mb-0">Danh sách thu nhập</h2>
    <a href="<?= BASE_URL ?>/incomes/create" class="btn btn-success btn-sm">
        <i class="bi bi-plus-lg me-1"></i> Thêm thu nhập
    </a>
</div>

<!-- Filter + Sort -->
<form method="GET" action="<?= BASE_URL ?>/incomes" class="card mb-3">
    <div class="card-body py-2">
        <div class="row g-2 align-items-center">
            ...
            <div class="col-12 col-sm-auto d-flex gap-2 align-items-end">
                ...
                <a href="<?= BASE_URL ?>/incomes" class="btn btn-sm btn-outline-secondary">Xoá lọc</a>
            </div>
        </div>
    </div>
</form>
...
<?php if (empty($items)): ?>
    <div class="text-center py-5 text-muted">
        ...
        Chưa có thu nhập nào<?= $filterMonth ? " trong tháng {$filterMonth}" : '' ?>.
        <a href="<?= BASE_URL ?>/incomes/create">Thêm ngay</a>
    </div>
<?php else: ?>
<div class="card">
    <div class="table-responsive">
        <table class="table table-hover align-middle mb-0">
            ...
            foreach ($items as $item):
                $color = $item['category_color'] ?? '#16a34a';
            ?>
            <tr>
                ...
                <td class="text-center">
                    <div class="d-inline-flex gap-1">
                        <a href="<?= BASE_URL ?>/incomes/<?= (int)$item['id'] ?>/edit"
                           class="btn btn-sm btn-outline-secondary py-0 px-2">
                            <i class="bi bi-pencil"></i>
                        </a>
                        <form method="POST"
                              action="<?= BASE_URL ?>/incomes/<?= (int)$item['id'] ?>/delete"
                              class="d-inline">
                            <input type="hidden" name="csrf_token"
                                   value="<?= htmlspecialchars($csrfDel, ENT_QUOTES) ?>">
                            <button type="submit"
                                    class="btn btn-sm btn-outline-danger py-0 px-2"
                                    data-confirm="Xoá thu nhập này?">
...
<div class="mt-3"><?= $pager->render('/incomes') ?></div>
<?php endif; ?>
<?php endif; ?>
